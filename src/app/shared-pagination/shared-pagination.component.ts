...
export class SharedPaginationComponent implements OnInit{
  ...
  public filters: any = {};
  public hideShowRecoreds: any = false;
  ...
  ngOnInit() {
    this.paginationValues.subscribe((value) => {
      if (value && value.type == 'page-init') {
        ...
        if (value.recordLimit) {
          this.recordLimit = value.recordLimit;
          this.selectedLimit = value.recordLimit;
        }
        if (value.hideShowRecoreds) {
          this.hideShowRecoreds = true;
        }
  ...
  calcNumberOfPage() {
    this.iteration =
      this.recordLimit == 0 ? 0 : this.totalTableRecords / this.recordLimit;
    if (this.iteration < 0) {
      this.iteration = 0;
